...
template<size_t N>
inline Matrix_slice<N>::Matrix_slice(size_t s, std::initializer_list<size_t> exts):start{s}
{
...
template<size_t N>
inline Matrix_slice<N>::Matrix_slice(size_t s, std::initializer_list<size_t> exts, std::initializer_list<size_t> strs) :start{s}
{
	static_assert(exts.size() == N, "");
	int product = 1;  // 初始化累积乘积为1
