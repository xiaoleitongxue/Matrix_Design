...
public:
  ...
  const Matrix_slice<1> &descriptor() const { return desc; }
...
public:
  ...
  explicit Matrix<T, N>(Matrix_initializer<T, N>);   // initializer from list
  Matrix<T, N> &operator=(Matrix_initializer<T, N>); // assign from list

  template <typename... Exts> // specify the extents
  Matrix(Exts... exts);       // init from dims

  template <typename U>
  Matrix<T, N>(std::initializer_list<U>) =
      delete; // don't use {} except for elements
...
Matrix<T, N>::Matrix(Exts... exts)
    : desc{static_cast<size_t>(exts)...}, elems(desc.size) {}
