...
struct Matrix_slice
{
    ...
    Matrix_slice(size_t start,
                 std::initializer_list<size_t> extents); // extents
    ...
    template <typename... Dims, typename = std::enable_if<matrix_impl::All(
                                    std::is_convertible<Dims, size_t>()...)>>
...
inline size_t Matrix_slice<N>::operator()(Dims... dims) const
{
    ...
    size_t args[N]{
        static_cast<size_t>(dims)...}; // copy arguments into an array
    return std::inner_product(args, args + N, strides.begin(),
...
constexpr bool Requesting_slice()
{
    return matrix_impl::All(
               (Convertible<Args, size_t>() || Same<Args, Slice>())...) &&
...
size_t do_slice_dim(const Matrix_slice<N>& os, Matrix_slice<N>& ns, const T& s)
{
    ...
    const size_t start = i * os.strides[current_dim];
    return start;
}
                                      ...
                                      Matrix_slice<N - 1>& row)
{
    ...
    std::copy(desc.extents.begin() + 1, desc.extents.end(),
              row.extents.begin());
    std::copy(desc.strides.begin() + 1, desc.strides.end(),
              row.strides.begin());
